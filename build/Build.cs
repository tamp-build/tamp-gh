...
/// <summary>
/// tamp-gh's self-hosted build script. Drives the
...
#pragma warning restore CS0649
    ...
    Target Test => _ => _
        ...
        .Description("Run the unit test suite (excludes integration tests, which require gh installed).")
        .Executes(() => DotNet.Test(s => s
            .SetProject(RootDirectory / "tests" / "Tamp.GitHubCli.V2.Tests" / "Tamp.GitHubCli.V2.Tests.csproj")
            .SetConfiguration(Configuration)
            .SetNoBuild(true)
    ...
    Target Pack => _ => _
        ...
        .Description("Pack the Tamp.GitHubCli.V2 NuGet package into ./artifacts.")
        .Executes(() => DotNet.Pack(s =>
        {
            s.SetProject(RootDirectory / "src" / "Tamp.GitHubCli.V2" / "Tamp.GitHubCli.V2.csproj");
    ...
    readonly string SonarProjectKey = "tamp-build_tamp-gh";
